...
1. Studio → **Deployment**
2. เลือก **target** ให้ตรงเส้น:
   - กล้อง/ไมค์ → **Arduino UNO Q** (ได้ไฟล์ `.eim` สำหรับฝั่ง Linux)
     - ⚠️ อย่าเลือก "Linux (x86 / AARCH64)" เฉยๆ ไม่งั้นเจอ **error 32-bit**
   - Modulino → **Arduino library** (ได้ `.zip` เอาไป import ในสเก็ตช์)
3. **Build** → ดาวน์โหลด · จดไว้ว่านี่คือ build ของ **V1** (ไว้เทียบ V2)

### 6.2 รัน — กล้อง/ไมค์ (ฝั่ง Linux)

**วิธีเร็วสุด เช็กก่อนเข้า App Lab** — บน shell ของบอร์ด:
```bash
edge-impulse-linux-runner
```
- ดึง model ของ project มารัน + เปิด **live classification**
- เปิดเบราว์เซอร์ไปที่ `http://<IP ของบอร์ด>:4912` → เห็นภาพ/เสียง + ผลทำนายสด
- (IP ดูได้จากแถบล่างของ App Lab หรือ `ip addr`)

แล้วค่อยเข้า **App Lab**: New App → เพิ่ม **AI model brick** → เลือก input ให้ตรง → **Run**

### 6.3 รัน — Modulino (ฝั่ง MCU)

EI build เป็น **Arduino library (.zip)**:
1. App Lab → Sketch → **import library** (.zip ที่โหลดมา)
...
### 6.4 Quick Test — อ่านผล runtime ให้เป็น

ป้อน input จริง 3–5 ครั้ง แล้วดูเลขพวกนี้:
...
| อาการ | วิธีแก้ |
|---|---|
| error 32-bit / `.eim` ลงไม่ได้ | target ต้องเป็น **Arduino UNO Q** ไม่ใช่ "Linux" เฉยๆ |
| memory / build error | model ใหญ่ไป → ลด model size / ลด class / ลด window |
| รันแล้วไม่เห็นภาพ-เสียง | input ไม่ตรง หรือกล้อง/ไมค์หลุด → [troubleshooting.md](troubleshooting.md) ข้อ 2 |
| ทาย class เดิมตลอด | input ไม่เปลี่ยนจริง / data ไม่ balance |

### 6.6 (ต่อยอด) Prototype — ให้ตอบเป็นไฟ/เสียง
